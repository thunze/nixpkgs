...
- `google-fonts` has been split into individual font packages under the namespace `google-fonts`. The locations of font files have changed from `$out/share/fonts/truetype/*.ttf` to `$out/share/fonts/truetype/google-fonts/<font-name>/*.ttf`. To obtain a list of all font packages under `google-fonts`, use `builtins.filter lib.isDerivation (builtins.attrValues google-fonts)`.
